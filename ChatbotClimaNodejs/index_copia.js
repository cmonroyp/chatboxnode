...
app.post("/api/tiempo", (req, res) => {
  ...
  //let ubicacion = req.body.result.parameters["ciudad"];
  ...
  //console.log("Ciudad enviada:", req.body.ciudad);
  //let ubicacion = req.queryResult.parameters["ciudad"];
  ...
  request(url, function(error, response, body) {
    ...
    if (_body.cod === "200") {
      ...
      //   _response.speech = `La temperatura prevista para el día ${fecha} (${
      //     _body.list[0].dt_txt.split(" ")[1]
      //   }) en ${_body.city.name} es de ${temperatura.toFixed(1)} grados `;
      ...
      // //   console.log("respuesta:", _response);
      //res.status(_body.cod).send(_response);
      //   console.log(
      //     "respuesta enviada:",
      //     res.status(_body.cod).send(_response.displayText)
      //   );
    } else {
      // ERROR!!!
